...
export function BusinessCard({ business, horizontal = false, mobileCompact = false, dense = false, homeRail = false }: { business: Business; horizontal?: boolean; mobileCompact?: boolean; dense?: boolean; homeRail?: boolean }) {
  ...
  return (
    <article className={cn("group overflow-hidden rounded-[18px] border border-[#E8E8EE] bg-white transition hover:-translate-y-0.5 hover:shadow-[0_14px_35px_rgba(35,24,75,.1)]", horizontal && "flex p-3 hover:translate-y-0", mobileCompact && "max-md:min-h-[106px] max-md:rounded-xl max-md:p-1.5", dense && "md:rounded-xl md:p-2", homeRail && "min-h-[136px] rounded-[20px] border-[#E6E1F2] p-2.5 shadow-[0_8px_28px_rgba(40,27,91,.06)] hover:-translate-y-0.5 hover:border-[#D5CCF6] hover:shadow-[0_14px_34px_rgba(40,27,91,.11)]") }>
      <Link href={detailHref} className={cn("relative block overflow-hidden bg-[#ECECF1]", horizontal ? "h-[116px] w-[120px] shrink-0 rounded-xl" : "aspect-[1.65]", mobileCompact && "max-md:h-[94px] max-md:w-[100px] max-md:rounded-lg", dense && "md:h-[86px] md:w-[98px] md:rounded-lg", homeRail && "h-[116px] w-[108px] rounded-[15px] sm:w-[124px]") }>
        <Image src={business.image} alt={`${business.name} salon görünümü`} fill className="object-cover transition duration-500 group-hover:scale-[1.03]" sizes={homeRail ? "124px" : horizontal ? "120px" : "(max-width: 768px) 50vw, 280px"} />
        {business.sponsored && <Badge tone="gray" className="absolute left-2 top-2 bg-white/90">Sponsorlu</Badge>}
      </Link>
      <div className={cn("relative min-w-0 p-3.5", horizontal && "flex flex-1 flex-col py-1 pr-1", mobileCompact && "max-md:pl-2.5 max-md:pr-1 max-md:pt-0.5", dense && "md:py-0 md:pl-3", homeRail && "py-1.5 pl-3 pr-1") }>
        <button type="button" aria-label={favorite ? "Favorilerden çıkar" : "Favorilere ekle"} onClick={() => void toggleFavorite()} disabled={saving} className={cn("absolute right-3 top-3 z-10 grid h-8 w-8 place-items-center rounded-full bg-white/95 shadow-sm disabled:opacity-60", mobileCompact && "max-md:right-0 max-md:top-0 max-md:h-7 max-md:w-7 max-md:bg-transparent max-md:shadow-none", dense && "md:right-0 md:top-0 md:h-7 md:w-7 md:bg-transparent md:shadow-none", homeRail && "right-0 top-0 h-7 w-7 bg-[#F7F5FC] shadow-none") }>
        ...
        <div className={cn("mt-3 flex items-center justify-between", horizontal && "mt-auto", mobileCompact && "max-md:mt-1.5") }>
          <Badge className={cn(mobileCompact && "max-md:max-w-[130px] max-md:truncate max-md:px-1.5 max-md:py-1 max-md:text-[9px]", dense && "md:max-w-[155px] md:truncate md:px-1.5 md:py-1 md:text-[9px]", homeRail && "max-w-[125px] truncate px-2 py-1 text-[9px]")}><Clock3 className={cn("mr-1 h-3 w-3", mobileCompact && "max-md:hidden", dense && "md:hidden", homeRail && "hidden")} /> {business.nextAvailable}</Badge>
